# Integrating an existing project with geniusDebug
...
- Run geniusDebug's uploader in CI so frames symbolicate — see [DEPLOY.md](DEPLOY.md) §5.

**Drop-in reference:** the whole wiring (client/server/edge config, instrumentation, global-error,
tunnel route, remote kill switch, `withSentryConfig`) is in **`taskip-integration/`** — copy it in and
adjust paths.
...
Because the ingest is envelope-based, **just set the SDK's DSN to a geniusDebug project DSN**. No tunnel,
...
JS events; symbolication is skipped for non-JS (they already have real file paths). Laravel is scheduled
for **v2** (SRS §12) — client-config only, no backend change.

---

## 4. Remote kill switch (recommended)

Point your SDK gate at geniusDebug's config endpoint so you can disable/throttle it **without a redeploy**:
...
- Pin the Sentry SDK **major** version. The envelope payload is the contract geniusDebug's ingest parses;
  an SDK major upgrade is a reviewed change.
