...
- api+web+workers typecheck clean. 18 worker tests green (4 new — multi-map resolution, unmatched-frame fallback, webpack scheme stripping, Next-internal in-app exclusion), 14 api tests green.
...
- After deploy: call `POST /admin/recompute-culprits` once (admin) to backfill GD-180 for issues created before this fix.
