...
## Sprint 36 — Symbolication root-cause fix + edge-case hardening pass
**Status:** CODE COMPLETE (needs Coolify redeploy: api+workers+web; taskip-client hotfix branch needs merge+Vercel deploy)
**Started:** 2026-07-23

Root cause (taskip-client, not this repo): `scripts/upload-sourcemaps.mjs`'s debugId workaround regex matched a `//# debugId=<uuid>` COMMENT that only Sentry's own SaaS-upload pipeline writes (never runs here — no `SENTRY_AUTH_TOKEN` by design) — so every map got registered under a SHA-256 content-hash fallback that could never match a real event's `debug_meta.images[].debug_id`. Fixed to match the actual auth-independent runtime marker (`sentry-dbid-<uuid>`, injected by webpack's BannerPlugin regardless of auth). Verified live end-to-end (real prod error → real resolved source + line + code context) on both `taskip-client-git-main` and the `crm.xgenious.com` white-label domain (confirmed via Vercel Domains: all ~30 white-label domains share one Production deployment).

| Ticket | Title | Status | Priority | Description |
|--------|-------|--------|----------|-------------|
| GD-179 | [taskip-client] Fix debugId extraction — wrong marker format | DONE | HIGH | `scripts/upload-sourcemaps.mjs` `extractDebugIdFromJs()` now matches `sentry-dbid-<uuid>` (the real runtime marker) instead of the never-present `//# debugId=` comment. Branch `hotfix/genius-debug`, commit `1cdae61`, merged→main→deployed (`633a234`), live-verified. |
| GD-180 | Culprit frozen at pre-symbolication path | DONE | HIGH | `issues.culprit` was computed once in `normalize()` from the raw minified frame and never refreshed after `symbolicate()` resolved it — a fully symbolicated issue still showed the compiled chunk path as its title subtitle. `symbolicate.ts` now recomputes culprit from the resolved frames (FR-GRP-3). Forward-only; see GD-183 for backfill. |
| GD-181 | Multi-chunk stack traces only resolved one chunk | DONE | MED | `findMapR2Key`/`symbolicateWithMap` fetched+applied only the FIRST matching debug_id's map to the whole event — frames from any other chunk in the same stack stayed raw. New `findMapR2Keys` (all matches) + `symbolicateWithMaps` (tries every map per frame, first real mapping wins). 4 new tests. |
| GD-182 | Resolved paths leaked `webpack://_N_E/` + Next.js internals mis-tagged in-app | DONE | MED | Our uploader never runs Sentry's `rewriteSources` (SaaS-upload-only), so resolved `sources` kept the raw scheme prefix — broke both the in-app regex and any GitHub deep-link (`.../blob/sha/webpack://_N_E/...` — 404). `resolveFrame` now strips it; `FRAMEWORK_INTERNAL_RE` also excludes Next's own internal tree (`src/client\|server\|shared\|build\|export/`) from In App + GitHub linking — no repo is linked yet so this was latent, not yet user-visible. |
| GD-183 | Backfill culprit for pre-fix issues | DONE | MED | `issues.culprit` fix (GD-180) is forward-only. New admin-only `POST /admin/recompute-culprits` recomputes it from each issue's already-stored latest-event frames, org-wide. Safe to re-run. |
| GD-184 | Trace page "Error in this trace" always showed "(no message)" | DONE | HIGH | User screenshot. `events.message` is only set by SDK `captureMessage()` — `captureException()` (nearly all real errors) leaves it null. `misc.controller.ts` `GET /traces/:id` now falls back to the issue title (both the per-error list and `meta.leadMessage`). |
| GD-185 | Surface which artisan command/job crashed | DONE | HIGH | User screenshot: a Laravel queue-worker crash's stack trace never showed the triggering command. Data was already captured (sentry-laravel's `command_info` breadcrumb + a `command` tag, confirmed live: `"queue:work"` / full `artisan queue:work database --sleep=3...` invocation) but not surfaced anywhere on the issue page. `IssueDetail.tsx` Highlights now shows the `command` tag when present (auto-shown, not pin-gated, so JS issues that never have it aren't cluttered). |
| GD-186 | [taskip-client] Server + Edge errors never reached geniusDebug | DONE | MED | `sentry.server.config.ts`/`sentry.edge.config.ts` only pointed at the SaaS DSN — API routes/RSC/middleware errors were invisible to geniusDebug; only the browser SDK (`instrumentation-client.ts`) dual-sent. Same `makeMultiplexedTransport` pattern added to both. Branch `hotfix/genius-debug`, commit `edcce4c`. **Watch item**: once server errors start arriving, confirm what `platform` value Node/Edge SDK reports — `symbolicate.ts` only maps-applies when `platform === 'javascript'` (FR-MAP-10); if Node reports e.g. `'node'`, server stack traces won't symbolicate until that gate is broadened. |
| GD-187 | Culprit could be the literal string "Unknown" | DONE | HIGH | User screenshot: a PHP `php-laravel` issue's Suspect frame/culprit was unusable — root cause was a Laravel shutdown-captured fatal (sentry-php's `register_shutdown_function` path has no real backtrace to the trigger, only Laravel's own error-handler plumbing frames) whose *only* `in_app: true` frame had `file: "Unknown"` (a sentry-php placeholder for an unresolvable file) — culprit was literally rendering the string `"Unknown"`. New `packages/shared/culprit.ts` `computeCulprit()` (shared by workers `symbolicate.ts`/`normalize.ts` AND the api backfill, was duplicated/inconsistent before) skips SDK placeholder files (`"Unknown"`, `"[internal]"`, empty) and falls through: top in-app frame with a real path → any frame with a real path → previous culprit. Also noted for the user: the SQLSTATE breadcrumb shown alongside was almost certainly unrelated (leftover from an earlier job in the same long-lived Horizon worker process, not the actual trigger of *this* error) — a Laravel/Horizon breadcrumb-scoping nuance, not a geniusDebug bug. 4 new tests. |
| GD-188 | Reference uploader (setup guide) had the identical debugId bug | DONE | HIGH | `scripts/upload-sourcemaps.mjs` — the canonical uploader the setup guide tells every NEW project to copy — read the .map's `debugId`/`debug_id` field, which per GD-179's root cause is only populated by Sentry's auth-token-gated upload step the guide explicitly says not to configure. Any new project following the guide verbatim would hit the exact same silent, permanent symbolication failure this whole sprint was about. Fixed with the same `sentry-dbid-<uuid>`-from-JS-file extraction as GD-179 (falls back to the .map field, then a content hash). |
| GD-189 | Setup guide missing server/edge config + explicit sourcemaps flag | DONE | MED | `IntegrationGuide.tsx`'s JS/Next.js snippet only ever showed `sentry.client.config.ts` — a new project following it would have GD-186's exact client-only blind spot from day one. Added `sentry.server.config.ts`/`sentry.edge.config.ts` to the snippet, plus an explicit `sourcemaps: { disable: false }` in the `next.config` example (previously only a comment saying not to set it true). |
| GD-190 | Display-side twin of GD-187 — stack trace featured the same unresolvable frame | DONE | HIGH | User screenshot: even with GD-187's culprit fix, `StackTrace.tsx`'s "Crashed in" summary + `SuspectFrame` still blindly used the innermost frame — for the same PHP shutdown-fatal case that's sentry-php's `file:"Unknown", line:0` placeholder, so it read "Crashed in Unknown:0 in ...handleError" and suggested linking a GitHub repo (which can't help — there's no file to link). New shared `hasUsableFramePath()` (same skip-list as `computeCulprit`); "Crashed in" now picks the nearest usable frame, `SuspectFrame`'s fallback chain hardened the same way, a placeholder frame's expanded panel says the file simply couldn't be resolved instead of the generic GitHub hint, and line `0` no longer renders as a fake `:0`. |

### Sprint Stats
- Total: 11  /  TODO: 0  /  IN_PROGRESS: 0  /  DONE: 11  /  BLOCKED: 0
- Not fixed (assessed, deliberately deferred — flagged to user): `source_map_artifacts` grows unbounded (every deploy re-registers ~860 maps under fresh debugIds since the embedded commit SHA changes chunk content-hashes each build; no unique constraint) — not a correctness bug, `MAP_DAYS=90` retention already reclaims it eventually, so no migration added.
- api+web+workers typecheck clean. 22 worker tests green (8 new total — multi-map resolution, unmatched-frame fallback, webpack scheme stripping, Next-internal in-app exclusion, "Unknown"-placeholder culprit skip + fallback chain), 9 ingest + 14 api unaffected/green.
- Needs: Coolify redeploy (api + workers + web) for this repo; on taskip-client, merge `hotfix/genius-debug` → `main` (user manages, tests via Vercel preview first) for GD-179/186.
- After deploy: call `POST /admin/recompute-culprits` once (admin) to backfill GD-180/GD-187 for issues created before these fixes.

## Sprint 37 — Verify Sprint 36 symbolication fix locally + Local Dev badge + fix a web-crashing regression
**Status:** COMPLETE
**Started:** 2026-07-23

User reported 5 latest prod issues on Taskip-Client still showing unresolved/minified frames despite Sprint 36's fix being deployed. Investigation found 2 of the 5 were `localhost` dev-server errors (never minified, so they "looked fine" but never touched the source-map pipeline at all) — not evidence of anything. The other 3 were genuine prod failures.

| Ticket | Title | Status | Priority | Description |
|--------|-------|--------|----------|-------------|
| GD-191 | Local full-fidelity repro of the layout-chunk symbolication path | DONE | HIGH | Built the actual `taskip-client` app (`hotfix/genius-debug`) in production mode locally, temporarily repointed its DSN at local ingest (reverted after, branch/backups per user's safety rules), forced a real uncaught error whose crashing frame lives in the root `app/layout-<hash>.js` chunk (same chunk as the broken prod issue), uploaded real `.next` source maps to local geniusDebug. Result: **frame resolved cleanly** — `inApp:true`, `components/ErrorReporter.tsx:17`, real source context. Confirms the Sprint 36 fix works correctly; the 3 broken prod issues are pre-fix/stale-cache artifacts (symbolication is one-shot at ingest, no backfill exists for frame resolution — user explicitly declined building one this pass). No code fix needed for symbolication itself. |
| GD-192 | Local Dev badge on Issue Detail | DONE | MED | New `isLocalUrl()`/`LocalDevBadge` in `apps/web/src/components/ui.tsx` — checks the event URL's hostname (`localhost`/`127.0.0.1`/`::1`), platform-agnostic (works off `event.url`, same field for JS and PHP/Laravel). Wired into `IssueDetail.tsx` header next to the status chip. Scoped to Issue Detail only — the Issues feed's `IssueDto` has no per-row `url` field, adding it there needs a backend query change, not done this pass. |
| GD-193 | Fix apps/web crash: node:crypto import broke the entire dashboard | DONE | HIGH | Found while verifying GD-192 in-browser: `packages/shared/src/crypto.ts` imported `{ createHash, createCipheriv, ... }` from `node:crypto` as **named imports** and called `createHash` at module top-level. Vite's browser build stubs `node:crypto` with a Proxy that throws on any property access — and named-import destructuring triggers that access at **module-evaluation time**, not at the call site. GD-190 (today, Sprint 36) was the first time web code took a real *value* import from the `@geniusdebug/shared` barrel (`hasUsableFramePath`), which transitively pulled in `crypto.ts` — crashing the entire dashboard on load (blank page, `apps/web` completely broken). Fixed: switched to `import * as nodeCrypto from 'node:crypto'` + lazy `DEV_KEY` (only property-accessed inside functions that the browser never calls). Verified live: login page + Issue Detail render correctly post-fix. |

### Sprint Stats
- Total: 3  /  TODO: 0  /  IN_PROGRESS: 0  /  DONE: 3  /  BLOCKED: 0
- `packages/shared` + `apps/web` + `apps/ingest` + `apps/workers` + `apps/api` + `packages/db` all typecheck clean; 42 tests green. Browser-verified live: badge renders correctly on localhost-origin issues; logic double-checked against real prod hostnames (`taskip.xyz`, `crm.xgenious.com`) → correctly false.
- GD-193 is a real, currently-shipped-risk fix — needs `apps/web` redeploy on Coolify. Was silently broken since GD-190 landed earlier today (Sprint 36); caught only because this session happened to exercise the live dashboard right after.

## Sprint 38 — Issue-delete cascade gap + replay delete (single + bulk)
**Status:** COMPLETE
**Started:** 2026-07-24

| Ticket | Title | Status | Priority | Description |
|--------|-------|--------|----------|-------------|
| GD-194 | Fix issue delete: cascade replays + traces/spans (were orphaned) | DONE | HIGH | `issues.service.ts deleteMany` (GD-149) only deleted `events`+`issues` explicitly; `replays.issueId` is `onDelete:set null` (not cascade) so replay rows + R2 blobs survived a deleted issue, and `traces`/`spans` have no `issueId` at all so they were never touched — orphaned forever. Rewrote to: collect distinct `traceId`s from the issue's events, gather matching `replays` rows (by `issueId` OR `traceId`) + their `r2Prefix` keys, wrap deletes in a `db.transaction` (replays → events → orphan-only traces/spans → issues), then best-effort R2 `deleteObjects` after commit. A trace is only dropped if no surviving event (outside the delete) still references it. Added `deleteObjects()` to `apps/api/src/r2.ts` (mirrors the existing `apps/workers/src/r2.ts` pattern — was API-side read-only before). |
| GD-195 | Replay delete: single + bulk, with R2 blob cleanup | DONE | HIGH | No delete endpoint existed for replays at all. Added `DELETE /replays/:id` + `POST /replays/bulk/delete` (`misc.controller.ts`) — both resolve a given row id to its **whole session** (every segment sharing `replayId`+`projectId`, since one replay is split across many segment rows) before deleting, access-scoped via `accessibleProjectIds`. Web: `Replays.tsx` gained checkbox selection + select-all + a bulk toolbar (Delete) + a per-row ✕ delete button, following the same selection-toolbar pattern as Issues.tsx bulk actions (GD-149); `ReplayPlayer.tsx` (single-replay detail page) gained a "Delete replay" button in the metadata bar that navigates back to `/replays` on success. |

| GD-196 | Fix replay↔issue linking (dev-team complaint: "not properly linked") | DONE | HIGH | Root-caused 3 independent bugs in `replays.issueId` resolution, all in the pipeline traced for GD-132/169: (1) **race, no self-heal** — `replay_recording` items are separate envelopes/jobs from the `event` item and can finish first (cheap insert vs. symbolication); the replay's `issueId` lookup ran once at insert time against `events` that didn't exist yet → NULL forever, nothing ever revisited it. `processor.ts processEvent` now backfills any orphaned replay (`issueId IS NULL`) on the same `traceId`+`projectId` once the event resolves an issue. (2) **unscoped/nondeterministic match** — `processReplay`'s lookup had no `projectId` filter and no `ORDER BY` on a `LIMIT 1`, and only checked `trace_ids[0]` even though Sentry's `trace_ids` array isn't guaranteed led by the erroring trace; now scoped to `projectId`, matches against ALL of `trace_ids`, ordered `desc(timestamp)` for determinism. (3) **merge orphaned replays** — `issues.service.ts merge()` re-pointed `events.issueId` but never touched `replays.issueId`; deleting the merged-away source issue then hit the FK's `onDelete:'set null'` and silently NULLed every one of its replays. Now `merge()` also re-points `replays.issueId` before the delete. Added admin-only `POST /admin/recompute-replay-links` (mirrors GD-183's `recompute-culprits` backfill pattern, no web UI — call directly) to fix up replays already stuck orphaned from bug (1)/(2) before this fix shipped. |

### Sprint Stats
- Total: 3  /  TODO: 0  /  IN_PROGRESS: 0  /  DONE: 3  /  BLOCKED: 0
- api+web typecheck clean (full workspace `npm run typecheck` green); 42 tests still green (no regressions). Not browser-verified live this pass (no dev servers running) — logic verified by tracing the existing `recording()` segment-gathering pattern and the project-delete transaction pattern (GD-060) it mirrors.
- Needs api+workers redeploy on Coolify. After deploy, call `POST /admin/recompute-replay-links` once (admin) to backfill already-orphaned replays from before this fix, same as GD-183's `recompute-culprits`.
- Not fixed (flagged, out of scope this pass): `replays.eventId` column exists but is never written — the SDK's actual authoritative error↔replay correlator is `contexts.replay.replay_id` on the error event, which geniusDebug doesn't capture/persist at all (`SentryEventPayload.contexts` has no `replay` field). trace_id-based matching is a heuristic, not the real link; a proper fix means capturing `replay_id` on error events and matching on that instead.
- No taskip-client code changes shipped this sprint — all edits there were temporary (DSN swap + a throwaway test throw in `ErrorReporter.tsx`), reverted via `git checkout --` and diff-verified byte-identical against scratchpad backups before finishing.

## Sprint 39 — Real fix for replay↔issue linking (replay_id correlator)
**Status:** CODE COMPLETE (needs migration 0016 applied in prod + api/workers redeploy)
**Started:** 2026-07-24

User reported live on `debug.taskip.net`: replays still show "No linked issue" after Sprint 38's GD-196 deploy. Live-verified (browser, admin session): 2 prod replays both had `traceId: null`; ran `POST /admin/recompute-replay-links` — `checked:2, updated:0`. Root-caused the flagged gap from GD-196's own notes.

| Ticket | Title | Status | Priority | Description |
|--------|-------|--------|----------|-------------|
| GD-197 | Correlate replay↔event by `replay_id`, not `trace_id` | DONE | HIGH | Root cause: prod `taskip-client` runs `tracesSampleRate: 0.1`, so most page loads have no sampled transaction — Sentry Replay's `trace_ids` array (built only from *sampled* spans seen during the segment) is empty even though the error event itself always carries an ambient `contexts.trace` (Sentry attaches trace context regardless of sampling decision). GD-196's trace-based matching was therefore structurally unable to link most replays — confirmed live (`traceId: null` on both orphaned rows, event `contexts` also confirmed empty of any `replay` block). The real, sampling-independent correlator is `contexts.replay.replay_id`, which Sentry's Replay integration stamps on **every** event fired while a session is recording. Added `events.replay_id` column (migration `0016_loving_mephisto.sql`, indexed, additive/partition-safe); `normalize.ts` now extracts it from `contexts.replay.replay_id`; `processor.ts` stores it on insert and backfills orphaned replays by `replay_id` first (falls back to the GD-196 `trace_id` path for older data); `processReplay`'s issue lookup now tries `events.replay_id` before `trace_ids`; `admin/recompute-replay-links` mirrors the same replay_id-first/trace_id-fallback order. `trace_id` matching kept as the fallback (still correct when tracing *is* sampled). All workspaces typecheck clean; 45 tests green (9 ingest + 22 workers + 14 api), no regressions. SRS §7 events table updated. |
...
## Sprint 41 — Uploader build-time blowup (Vercel Build CPU Minutes > Vercel+Sentry combined)
**Status:** CODE COMPLETE (needs api redeploy here + user to merge/push taskip-client's hotfix/genius-debug → main → Vercel deploy)
**Started:** 2026-07-25

User screenshot: Vercel Usage showed Build CPU Minutes $72.88 as the top cost line, dwarfing everything else — sourcemap upload step taking 5-20min per build. Initial pass fixed only this repo's *reference* `scripts/upload-sourcemaps.mjs` (R2-direct contract) — turned out taskip-client's REAL script uses a different contract entirely (base64 batches of 50 → geniusDebug API → API does R2 upload server-side), so the real bottleneck was found + fixed in both places.

| Ticket | Title | Status | Priority | Description |
|--------|-------|--------|----------|-------------|
| GD-199 | Fix reference `scripts/upload-sourcemaps.mjs`: sequential R2 upload → bounded-concurrency + gzip | DONE | HIGH | `for (const mapPath of maps) { ...await uploadToR2... }` awaited one R2 `PutObjectCommand` at a time. Added `mapWithConcurrency()` worker pool (default 24, `SOURCEMAP_UPLOAD_CONCURRENCY` env), fixed a latent `_s3` init race (memoize the client *promise*), gzip the map body before PUT (`ContentEncoding: gzip`), parallelized the `.map` strip pass. Worker read side (`apps/workers/src/symbolicate.ts`) gained `decodeMapBytes()` — gunzip-decodes by magic-byte detection so old plain-JSON maps already in R2 still work, no backfill needed. 22 worker tests green. |
| GD-200 | Fix the REAL bottleneck: `POST /:projectId/releases/:release/upload` (admin.controller.ts) looped sequentially | DONE | HIGH | This is the endpoint taskip-client's actual uploader calls (batches of 50 base64 files) — it did `for (const f of body.files) { await s3.PutObject; await db.insert; }`, i.e. ~50 sequential R2 round-trips *per batch*, ×~18 batches. This — not the reference script — was the real cause of the 5-20min build. Added the same `mapWithConcurrency()` pool (default 24, `SOURCEMAP_UPLOAD_CONCURRENCY` env), batched the artifact-index insert into one `db.insert(...values(rows))` instead of one per file, and made the endpoint gzip-aware (`decodeMaybeGzip()` — detects magic bytes, decodes for JSON debugId parsing, stores original bytes, sets `ContentEncoding: gzip` on R2 only when the input actually was gzip so it stays correct against old non-gzip client versions too). api typecheck clean, 14 api tests green. |
| GD-201 | [taskip-client] Gzip each map before base64; batch dispatch stays sequential | DONE | HIGH | Companion client-side fix on branch `hotfix/genius-debug` (commits `10e3d2609`, `2a719e96e`; done via an isolated `git worktree` so the user's in-progress uncommitted work on `sharifur` branch was never touched). Each map gzipped (`node:zlib gzipSync`) before base64. Batch dispatch concurrency (`SOURCEMAP_BATCH_CONCURRENCY`) **defaults to 1** (sequential, unchanged from before) after local measurement showed 4-concurrent-batches was *slower* than the server fix alone — see GD-202. `node --check` clean. **Not pushed/merged — user explicitly asked to review + merge to main themselves.** |
| GD-202 | Locally measured, real numbers (against real R2, not mocked) | DONE | HIGH | Built a local test harness: 300 synthetic `.map`/`.js` pairs (realistic ~250KB each, 76MB total) fed through the real taskip-client script against a locally-run `api` (port 4092, isolated from the already-running unrelated dev services) hitting the **real** R2 bucket. Results: **old client + old server (true baseline) = 191s/300 files → extrapolates to ~9min/860, matching the user's reported 5-20min.** New server (concurrent R2 PUT + batched insert) + old sequential client = **17s** (~11x — the server fix is the dominant win). New server + a client that ALSO dispatched 4 batches concurrently = **73s — slower than the server fix alone** (client+server shared one machine/uplink in this test, ~96 concurrent uploads saturated it; unproven whether Vercel↔Coolify's real two-machine topology has the same ceiling, so shipped the safe/proven config instead of the unproven one). Final config (fixed server + sequential-gzip client) re-verified: **14.7s/300 files → ~42s/860 extrapolated, ~13x vs. true baseline.** Test harness + local test api process cleaned up afterward; `sharifur` branch's uncommitted work (30 modified + 20 untracked files) confirmed untouched throughout (git worktree isolation). |
