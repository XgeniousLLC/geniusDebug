...
### Sprint Stats
- Total: 2  /  TODO: 0  /  IN_PROGRESS: 0  /  DONE: 2  /  BLOCKED: 0
- Tests: 32 green (8 ingest + 14 workers + 10 api). Migration 0010 applied. DeepSeek key verified live (`deepseek-chat`→`deepseek-v4-flash`, JSON mode OK). **Set the key** (Settings→Integrations→DeepSeek or `DEEPSEEK_API_KEY` env), redeploy api+web. P2 grounding needs a linked GitHub repo (+ App install) to fetch source; else falls back to stored frames. P3 (multi-candidate+critique), P4 (human-approved draft PR) still parked.
