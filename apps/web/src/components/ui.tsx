...
export function Button({
  ...
  const sizes = size === 'sm' ? 'h-7 px-2.5 text-caption' : 'h-9 px-3.5 text-body';
...
export function LevelPill({ level }: { level: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 text-caption text-text-muted uppercase tracking-wide">
      <span className={`h-2 w-2 rounded-full ${LEVEL_COLORS[level] ?? 'bg-level-debug'}`} aria-hidden />
      {level}
    </span>
  );
}
...
export function StatusChip({ status, regressed }: { status: string; regressed?: boolean }) {
  return (
    <span className="inline-flex items-center gap-1.5">
      <span className={`rounded-full border px-2 py-0.5 text-caption font-medium capitalize ${STATUS_STYLE[status] ?? ''}`}>
        {status}
      </span>
      {regressed && (
...
/* --------------------------------- Tag ------------------------------------ */
export function Tag({ k, v, onClick }: { k: string; v: string; onClick?: () => void }) {
  return (
    <button
      onClick={onClick}
...
export function EmptyState({ title, hint, icon }: { title: string; hint?: string; icon?: React.ReactNode }) {
  return (
    <div className="flex flex-col items-center justify-center gap-2 rounded-md border border-dashed border-border bg-surface/50 py-16 text-center">
      ...
      {hint && <div className="max-w-md text-small text-text-muted">{hint}</div>}
    </div>
  );
}
