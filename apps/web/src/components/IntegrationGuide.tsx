...
export function IntegrationGuide({
  ...
  const keys = useQuery({
    ...
    queryFn: () =>
      api<{ publicKey: string; isActive: boolean }[]>(
...
// CAUTION if you later add a second (e.g. SaaS Sentry) DSN via a custom
// \`transport\` to mirror events to both: Sentry.makeFetchTransport and
// Sentry.makeMultiplexedTransport are BROWSER-SDK-ONLY — they don't exist on
// the server/edge builds. Calling them here throws synchronously inside
// Sentry.init(), which crashes edge middleware outright (Vercel
// MIDDLEWARE_INVOCATION_FAILED) and silently breaks server-side init.
// Import both from "@sentry/core" instead, and hand-roll the fetch transport
// (fetch is available in both the Node and edge runtimes):
//
//   import { createTransport, makeMultiplexedTransport } from "@sentry/core";
//   function makeFetchTransport(options) {
//     return createTransport(options, (request) =>
//       fetch(options.url, { method: "POST", body: request.body, headers: options.headers })
//         .then((r) => ({ statusCode: r.status, headers: {
...
export default withSentryConfig(nextConfig, {
  ...
  const sendEmail = useMutation({
    mutationFn: () =>
      api<{ sent: boolean; reason?: string; to: string }>(
        `/projects/${project.id}/setup/email`,
        {
          method: "POST",
          body: JSON.stringify({
            email: devEmail,
    ...
    onSuccess: (r) => {
      if (r.sent) setEmailMsg({ ok: true, text: `Sent to ${r.to}.` });
        ...
        setEmailMsg({
          ok: false,
          text: `Couldn't send (${r.reason ?? "email not configured"}). Use the mail-client link instead.`,
          mailto: buildMailto(devEmail, project.name, dsn, isPhp, note),
        });
    },
    onError: (e) =>
      setEmailMsg({
        ok: false,
        text: e instanceof ApiError ? e.message : "send failed",
      }),
  });

  const inp =
    "h-9 rounded-md border border-border bg-bg px-2.5 text-small text-text";
    ...
    <div className="flex flex-col gap-4">
        ...
        <div className="text-small">
          {project.setupCompletedAt ? (
            ...
            <span className="text-text-muted">
              Wire the SDK below, then mark this project complete.
            </span>
          )}
        </div>
        {project.setupCompletedAt ? (
      ...
      <ol className="ml-4 list-decimal space-y-1 text-small text-text-muted">
        {isPhp ? (
          <>
            ...
            <li>
              Publish config + stub .env:{" "}
              <code className="font-mono text-text">
                php artisan sentry:publish --dsn="{dsn}"
              </code>{" "}
              (creates{" "}
              <code className="font-mono text-text">config/sentry.php</code>).
            </li>
            <li>
              Register the handler in{" "}
              <code className="font-mono text-text">bootstrap/app.php</code>{" "}
              (Laravel 11+) or{" "}
              <code className="font-mono text-text">
                app/Exceptions/Handler.php
              </code>{" "}
              (Laravel ≤10) — see below.
            </li>
            <li>
              Set <code className="font-mono text-text">.env</code> and
              (optionally) the{" "}
              <code className="font-mono text-text">tracing</code> toggles in{" "}
              <code className="font-mono text-text">config/sentry.php</code>{" "}
              below.
            </li>
            <li>
              ...
              above 0 to get performance traces — this is what populates the
              query waterfall.
            </li>
            <li>
              Verify locally:{" "}
          ...
          <>
            <li>
              Install the SDK:{" "}
              <code className="font-mono text-text">npm i @sentry/nextjs</code>
            </li>
            <li>
            ...
            <li>
              Wrap <code className="font-mono text-text">next.config.js</code>{" "}
              with <code className="font-mono text-text">withSentryConfig</code>{" "}
              + tunnel route (see below).
            </li>
            <li>Deploy — events appear here within seconds.</li>
          </>
        )}
      ...
      <div>
        <div className="mb-1 flex items-center justify-between">
          <span className="text-caption uppercase text-text-faint">
            {isPhp ? ".env / config" : "Sentry.init + next.config"}
          </span>
          <button
      ...
      {isPhp && (
        <div className="rounded-md border border-accent/30 bg-accent/5 p-3 text-caption text-text-muted">
          <div className="mb-1 font-semibold text-text">
            What you get on an API-only Laravel project
          </div>
          <ul className="ml-4 list-disc space-y-1">
            <li>
              <span className="text-text">Query waterfall, yes</span> — with{" "}
              <code className="font-mono">sql_queries: true</code> the SDK
              auto-wraps every DB query as a span (SQL text, duration, source
              file:line via <code className="font-mono">sql_origin</code>). It
              shows in the Trace waterfall the same as JS spans, sorted by start
              time with per-span duration bars. Bound parameter values are
              hidden by default (
              <code className="font-mono">sql_bindings: false</code>) — flip on
              only if you're OK with that data reaching geniusDebug.
            </li>
            <li>
              Also auto-instrumented as spans: cache ops, outbound HTTP client
              calls, queue jobs, Blade view renders — all timed and nested under
              the request transaction.
            </li>
            <li>
              <span className="text-text">No session replay</span> — replay is a
              browser/DOM (rrweb) recording feature; a server-side API has no
              DOM to record. You still get: error + stack trace, breadcrumbs,
              request context (method/url/headers/user), and the full
              trace/query waterfall above.
            </li>
          </ul>
        </div>
      )}
      ...
      <div className="rounded-md border border-border bg-surface/50 p-3">
        <div className="mb-2 text-small font-semibold text-text">
          Send setup instructions to a developer
        </div>
        <div className="flex flex-wrap items-end gap-2">
          ...
          <label className="flex flex-1 flex-col gap-1">
            ...
            <input
              ...
              placeholder="Please wire this before Friday"
            />
          </label>
          <Button
            size="sm"
            variant="primary"
            disabled={sendEmail.isPending || !devEmail.trim()}
            onClick={() => {
              setEmailMsg(null);
              sendEmail.mutate();
            }}
        ...
        {emailMsg && (
          <div
            className={`mt-2 text-caption ${emailMsg.ok ? "text-status-resolved" : "text-level-error"}`}
          >
            {emailMsg.text}
            {emailMsg.mailto && (
              <a
                href={emailMsg.mailto}
      ...
      <div className="rounded-md border border-border bg-surface/50 p-3">
        <div className="mb-2">
          ...
          <div className="text-caption text-text-muted">
            Optional — link this project's repo so stack frames deep-link to the
            exact source line (FR-GH-1/3).
          </div>
        </div>
...
function SourceMapCallout({ project }: { project: GuideProject }) {
  ...
  const issue = useMutation({
    mutationFn: () =>
      api<{ token: string }>(`/projects/${project.id}/upload-token`, {
        method: "POST",
      }),
...
GENIUSDEBUG_ORG_TOKEN=<paste the token issued below>`;
  return (
    <div className="rounded-md border border-level-warning/30 bg-level-warning/5 p-3 text-caption text-text-muted">
      <div className="mb-1 font-semibold text-text">
        Set up source maps — or production errors stay minified
      </div>
      <p className="mb-2">
        Deployed JS is minified. Without this step, errors here look like{" "}
        <code className="font-mono text-text">Minified React error #422</code>{" "}
        with no real file, line, or variable names — undebuggable. Sentry's own
        SaaS upload is deliberately disabled for this project; geniusDebug
        uploads maps straight to your own R2 with a separate post-build step.
      </p>
      <ol className="ml-4 list-decimal space-y-1">
        <li>
          Add a post-build step in your Vercel project:{" "}
          <code className="font-mono text-text">
            node scripts/upload-sourcemaps.mjs
          </code>
        </li>
        <li>
          Set these env vars in Vercel (Project → Settings → Environment
          Variables):
          <div className="mb-1 mt-1 flex items-center justify-between">
            ...
            <button
              onClick={() => copy(sourceMapEnvBlock)}
              className="text-caption text-accent hover:underline"
            >
              {copied ? "copied ✓" : "copy"}
            </button>
          ...
          <div className="mt-1 text-text-faint">
            R2_* values are the same ones you connected under Settings →
            Integrations → Cloudflare R2 (server-side secrets — never returned
            to the client, so re-enter them here).{" "}
            <code className="font-mono text-text">RELEASE</code> doesn't need to
            be set — Vercel auto-provides{" "}
            <code className="font-mono text-text">VERCEL_GIT_COMMIT_SHA</code>{" "}
            and the script falls back to it automatically.
          </div>
        </li>
        <li>
          Issue{" "}
          <code className="font-mono text-text">GENIUSDEBUG_ORG_TOKEN</code>{" "}
          (shown once) — never Sentry's own auth token:
          {isAdmin ? (
            token ? (
              <pre className="mt-1 overflow-x-auto rounded-md border border-border bg-bg px-3 py-2 font-mono text-mono text-text">
                {token}
              </pre>
            ) : (
              <div className="mt-1">
            ...
            <span className="text-text-muted">
              {" "}
              ask an admin (Settings → General → Source Maps).
            </span>
          )}
          {issue.isError && (
            <div className="mt-1 text-level-error">
