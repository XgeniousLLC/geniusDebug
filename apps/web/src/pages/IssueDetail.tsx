...
export function IssueDetail() {
    ...
    try {
      ...
      return new Set(
        s
          ...
          : ["handled", "level", "transaction", "url", "trace"],
      );
    } catch {
  ...
  const act = useMutation({
    ...
    onError: (e: unknown, action) =>
      toast.error(`Couldn't ${action} ${shortId}: ${errMsg(e)}`),
  });
  const members = useQuery({
    queryKey: ["members"],
    queryFn: () => api<{ id: string; name: string }[]>("/members"),
  });
  const assign = useMutation({
    ...
    onError: (e: unknown) =>
      toast.error(`Couldn't update assignee: ${errMsg(e)}`),
  });
  const suspect = useQuery({
    ...
    queryFn: () =>
      ...
      }>(`/github/issues/${shortId}/suspect-commits`),
  });
  const createIssue = useMutation({
    mutationFn: () =>
      api<{ url: string }>(`/github/issues/${shortId}/create`, {
        method: "POST",
      }),
    onSuccess: (r) => {
    ...
    onError: (e: unknown) =>
      toast.error(`Couldn't create GitHub issue: ${errMsg(e)}`),
  });
  ...
  const [selectedReplay, setSelectedReplay] = React.useState<string | null>(
    null,
  );
  ...
  const genSuggest = useMutation({
    mutationFn: (refresh: boolean) =>
      api<{ suggestion: FixSuggestion | null; reason?: string }>(
        `/issues/${shortId}/suggest`,
        {
          method: "POST",
    ...
    onSuccess: (r) => {
      if (r.suggestion) {
        qc.setQueryData(
          ["suggest", shortId],
          (prev: SuggestResponse | undefined) => ({
  ...
  const openPr = useMutation({
    mutationFn: (suggestionId: string) =>
      api<{ url: string }>(`/issues/${shortId}/suggest/pr`, {
        method: "POST",
  ...
  const togglePr = useMutation({
    mutationFn: (enabled: boolean) =>
      api<{ prEnabled: boolean }>(`/issues/${shortId}/suggest/pr-enabled`, {
        method: "POST",
    ...
    onSuccess: (r) => {
      qc.setQueryData(
        ["suggest", shortId],
        (prev: SuggestResponse | undefined) =>
          prev ? { ...prev, prEnabled: r.prEnabled } : prev,
      );
      toast.success(
        r.prEnabled ? "Draft PRs enabled for this repo" : "Draft PRs disabled",
      );
    },
      ...
      <div className="p-6">
        <div className="flex flex-col items-center justify-center gap-3 rounded-md border border-dashed border-border bg-surface/50 py-20 text-center">
          ...
          <div className="max-w-md text-small text-text-muted">
            It may have been merged into another issue, deleted, or the link is
            for a different project/environment.
          </div>
          <Link to="/issues">
  ...
  const contexts = (event?.contexts ?? {}) as Record<
    string,
    { name?: string; version?: string; model?: string }
  >;
  ...
  return (
    <div className="w-full px-4 py-5 sm:px-6">
      ...
      <div className="mb-3 flex items-center gap-1.5 font-mono text-caption text-text-faint">
        <Link to="/issues" className="hover:text-accent">
          Issues
        </Link>
        <span>/</span>
      ...
      <div className="mb-4 flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="mb-1 flex items-center gap-2">
            ...
            {isLocalUrl(event?.url) && <LocalDevBadge />}
          </div>
          ...
          <div className="mt-1 flex items-center gap-2 font-mono text-small text-text-muted">
            ...
            <span className="text-text-faint">
              first seen {timeAgo(issue.firstSeen)} ago
            </span>
          </div>
        </div>
        <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
          ...
          {issue.status === "resolved" ? (
            <Button size="sm" onClick={() => act.mutate("unresolve")}>
              <CheckIcon size={13} /> Unresolve
            </Button>
          ) : issue.status === "archived" ? (
            <Button size="sm" onClick={() => act.mutate("unarchive")}>
              <ArchiveIcon size={13} /> Unarchive
            </Button>
          ) : issue.status === "muted" ? (
            <Button size="sm" onClick={() => act.mutate("unmute")}>
              <BellOffIcon size={13} /> Unmute
            </Button>
          ) : (
            <Button
              size="sm"
          ...
          {issue.status !== "archived" && (
            <Button size="sm" onClick={() => act.mutate("archive")}>
              <ArchiveIcon size={13} /> Archive
            </Button>
          )}
          {issue.status !== "muted" && (
            <Button size="sm" onClick={() => act.mutate("mute")}>
              <BellOffIcon size={13} /> Mute
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <div className="min-w-0">
            ...
            <Card className="mb-4 p-4">
              <div className="mb-2 flex items-center justify-between">
                ...
                <span className="text-caption text-text-faint">
                  {issue.timesSeen} total · first seen{" "}
                  {timeAgo(issue.firstSeen)} ago
                </span>
              </div>
          ...
          <Card className="mb-4 p-4">
            <div className="mb-2 flex items-center justify-between">
              ...
              <button
                onClick={() => setEditingHi((e) => !e)}
                className="text-caption text-text-faint hover:text-accent"
              >
                {editingHi ? "Done" : "Edit"}
              </button>
            </div>
            {editingHi && (
              <div className="mb-3 flex flex-wrap gap-3 rounded-md border border-border bg-bg px-3 py-2 text-caption">
                {(
                  ["handled", "level", "transaction", "url", "trace"] as const
                ).map((k) => (
                  <label
                    key={k}
                    className="flex items-center gap-1.5 capitalize text-text-muted"
                  >
                    <input
                      type="checkbox"
                      checked={pinned.has(k)}
                      onChange={() => togglePin(k)}
                    />{" "}
                    {k}
                  </label>
                ))}
            ...
            <div className="grid grid-cols-1 gap-x-6 gap-y-2 text-small sm:grid-cols-2">
              {pinned.has("handled") && (
                <Highlight
                  k="handled"
                  v={event ? (event.handled ? "true" : "false") : "—"}
                />
              )}
              {pinned.has("level") && (
                <Highlight k="level" v={event?.level ?? issue.level} />
              )}
              {pinned.has("transaction") && (
              ...
              {event?.tags?.command && (
                <Highlight k="command" v={event.tags.command} mono />
              )}
              {pinned.has("trace") && (
              ...
              return (
                ...
                >
                  ...
                  {t === "events" && events.length > 1 && (
                    <span className="rounded-full bg-surface-2 px-1.5 text-caption text-text-muted">
                      {events.length}
                    </span>
                  )}
                  {t === "replay" && (
                    <span className="rounded-full bg-accent/15 px-1.5 text-caption text-accent">
                      {replayCount}
                    </span>
                  )}
                </button>
          ...
          {tab === "tags" &&
            (() => {
              ...
              put(
                "handled",
                event ? (event.handled ? "yes" : "no") : undefined,
              );
              ...
              if (c.browser?.name)
                put(
                  "browser",
                  `${c.browser.name}${c.browser.version ? ` ${c.browser.version}` : ""}`,
                );
              put("browser.name", c.browser?.name);
              if (c.os?.name)
                put(
                  "os",
                  `${c.os.name}${c.os.version ? ` ${c.os.version}` : ""}`,
                );
              put("os.name", c.os?.name);
              if (c.device?.family || c.device?.model)
                put("device", c.device?.family ?? c.device?.model);
              const merged = { ...derived, ...(event?.tags ?? {}) };
              const entries = Object.entries(merged);
              return (
                <div className="flex flex-wrap gap-2">
                  {entries.length === 0 && (
                    <span className="text-small text-text-muted">No tags.</span>
                  )}
                  {entries.map(([k, v]) => (
          ...
          {tab === "context" && (
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                {(() => {
                  const all = (event?.contexts ?? {}) as Record<
                    string,
                    Record<string, unknown>
                  >;
                  const cards = Object.entries(all).filter(
                    ([, v]) => v && typeof v === "object",
                  );
                  const trace = event?.traceId
                    ? {
                        trace_id: event.traceId,
                        span_id: event.spanId ?? undefined,
                        status: (all.trace?.status as string) ?? undefined,
                      }
                    : null;
                  ...
                  return (
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                      {Object.keys(user).length > 0 && (
                        <ContextCard title="User" obj={user} />
                      )}
                      {cards.map(([k, v]) => (
                      ...
                      {cards.length === 0 &&
                        ...
                        !trace && (
                          <span className="text-small text-text-muted">
                            No context captured for this event.
                          </span>
                        )}
                    </div>
                  );
            ...
            <>
              {events.length > 1 && (
                <div className="mb-2 text-caption text-text-faint">
                  {events.length} occurrences of this error — each trigger is
              ...
              <div className="overflow-hidden rounded-md border border-border">
                {events.map((e, i) => {
                  const hasReplay =
                    !!e.traceId &&
                    (issueReplays.data ?? []).some(
                      (rep) => rep.traceId === e.traceId,
                    );
                  return (
                    <button
                      key={e.id}
                    ...
                    >
                      <span className="flex items-center gap-2">
                        ...
                        {hasReplay && (
                          <span
                            title="Replay captured for this occurrence"
                            className="flex items-center gap-1 rounded-full bg-accent/15 px-1.5 py-0.5 text-caption text-accent"
                          >
          ...
          {tab === "replay" &&
            ...
            (() => {
              ...
              const linkedToOccurrence =
                ...
                active.traceId === currentEvent.traceId;
              return (
                <div>
                  <div className="mb-3 flex items-center justify-between">
                    <h2 className="text-h2 font-semibold">Session replays</h2>
                    <span className="text-caption text-text-faint">
                      {list.length} session{list.length > 1 ? "s" : ""} · newest
                      first
                    </span>
                  </div>
                  {linkedToOccurrence ? (
                    ...
                    <div className="mb-2 text-caption text-text-faint">
                      Showing the most recent session — no replay was captured
        ...
        <div className="flex flex-col gap-4">
          ...
          {suspect.data?.available &&
            (suspect.data.commits?.length ?? 0) > 0 && (
              <Card className="p-4">
                <div className="mb-2 text-h2 font-semibold">
                  Suspect commits
                </div>
                <div className="flex flex-col gap-1.5">
                  {suspect.data.commits!.slice(0, 3).map((c) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="block text-caption hover:text-accent"
                    >
          ...
          <SuggestCard
            ...
            onOpenPr={(id) => {
              if (
                window.confirm(
                  "Open a DRAFT pull request applying this patch to a new branch? It will NOT be merged.",
                )
              )
                openPr.mutate(id);
            }}
                ...
                {similar.data!.map((s) => (
                  ...
                  >
                    <span
                      ...
                      style={{ background: simColor(s.score) }}
                    />
          ...
          <Card className="p-4">
            ...
            {activity.length === 0 && (
              <div className="text-small text-text-muted">No activity yet.</div>
            )}
            {activity.map((a) => (
              <div
                key={a.id}
...
function Highlight({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span
        className={`truncate text-text ${mono ? "font-mono text-mono" : ""}`}
      >
        {v}
      </span>
    </div>
...
function ContextCard({
  ...
  return (
    <Card className="p-3">
      <div className="mb-2 text-caption font-semibold uppercase tracking-wide text-text-faint">
        {title}
      </div>
...
function HttpRequestCard({ request }: { request: Record<string, unknown> }) {
  ...
  const shown = Object.entries(headers).filter(
    ([k]) => !/cookie|authorization/i.test(k),
  );
  return (
    <Card className="col-span-full p-4">
      <div className="mb-2 flex items-center gap-2">
        <span className="rounded bg-accent/15 px-1.5 py-0.5 font-mono text-caption font-semibold text-accent">
          {method}
        </span>
        {url && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="truncate font-mono text-small text-text hover:text-accent"
          >
            {url}
          </a>
        )}
      ...
      {shown.length > 0 && (
        <div className="rounded-md border border-border">
          <div className="border-b border-border px-3 py-1.5 text-caption font-semibold uppercase tracking-wide text-text-faint">
            Headers
          </div>
          {shown.map(([k, v]) => (
            <div
              key={k}
              className="grid grid-cols-[140px_1fr] gap-3 border-b border-border px-3 py-1.5 text-small last:border-0"
            >
              <span className="text-text-muted">{k}</span>
              <span className="break-all font-mono text-mono text-text">
                {String(v)}
              </span>
            </div>
          ))}
...
function ShareModal({
  ...
  const share = useMutation({
    ...
    onSuccess: (r) => {
      ...
      toast.success(
        r.shareToken ? "Public link created" : "Public link disabled",
      );
    },
      ...
      <Card className="w-full max-w-xl p-0">
        ...
        >
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              variant="primary"
              onClick={() => copy(publicUrl ?? privateUrl, "link")}
            >
          ...
          <div className="flex items-center justify-between border-t border-border pt-4">
            <div>
              ...
              <div className="text-caption text-text-faint">
                Read-only view for anyone outside your org.
              </div>
            </div>
            <button
            ...
            >
              <span
                className={`absolute top-0.5 h-5 w-5 rounded-full bg-white transition-all ${token ? "left-[22px]" : "left-0.5"}`}
              />
            </button>
          </div>
        </div>
...
function TracePreview({
  ...
  const q = useQuery({
    queryKey: ["trace-preview", traceId],
    queryFn: () => api<TracePreviewResp>(`/traces/${traceId}`),
  });
  ...
  return (
    <Card className="mb-4 p-4">
      <div className="mb-3 flex items-center justify-between">
        <button
          onClick={() => setOpen((o) => !o)}
          className="flex items-center gap-2 text-h2 font-semibold"
        >
          <ChevronDownIcon
            size={16}
        ...
        <button
          onClick={onOpenFull}
          className="rounded-md border border-border bg-surface px-2.5 py-1 text-caption text-text-muted hover:text-text"
        >
          View Full Trace →
        </button>
      </div>
        ...
        <div className="overflow-hidden rounded-lg border border-border">
          {hiddenSpans > 0 && (
            <div className="border-b border-border bg-surface px-3 py-1.5 text-caption text-text-faint">
              {hiddenSpans} span{hiddenSpans === 1 ? "" : "s"} · {errors.length}{" "}
              error{errors.length === 1 ? "" : "s"} in trace
            </div>
          )}
          {errors.map((e) => {
            const isCurrent = e.issueId === currentIssueId;
            const left = e.timestamp
              ? ((Date.parse(e.timestamp) - t0) / span) * 100
              : 0;
            return (
              <div
                key={e.id}
              ...
              >
                <div className="flex min-w-0 items-center gap-2">
                  <span className="shrink-0 text-caption font-semibold text-level-error">
                    Error —
                  </span>
                  <span className="truncate text-small text-text">
                    {e.message ?? shortOf.get(e.issueId) ?? "error"}
                  </span>
                </div>
...
function Breadcrumbs({ crumbs }: { crumbs: Array<Record<string, unknown>> }) {
  if (!crumbs || crumbs.length === 0)
    return <span className="text-small text-text-muted">No breadcrumbs.</span>;
  ...
  const icon = (category?: string, type?: string) => {
    ...
    if (c.startsWith("http") || c === "fetch" || c === "xhr")
      return <GlobeIcon size={sz} />;
    if (c === "error" || type === "error")
      return <AlertTriangleIcon size={sz} />;
  ...
  return (
    <div className="relative flex flex-col">
      ...
      {crumbs.map((b, i) => {
        ...
        const category =
          (b.category as string | undefined) ?? (b.type as string | undefined);
        const message =
          (b.message as string | undefined) ??
          (b.data && typeof b.data === "object" ? JSON.stringify(b.data) : "");
        ...
        return (
          <div key={i} className="relative flex items-start gap-3 py-1.5 pl-0">
            <span
              className={`z-10 mt-1 grid h-4 w-4 shrink-0 place-items-center rounded-full ${dot(level, category)} text-[8px] text-white`}
            >
            ...
            <div className="min-w-0 flex-1 border-b border-border pb-1.5">
              <div className="flex items-center justify-between gap-2">
                <div className="flex min-w-0 items-center gap-2">
                  <span className="shrink-0 rounded bg-surface-2 px-1.5 py-0.5 font-mono text-caption text-text-muted">
                    {category ?? "log"}
                  </span>
                  <span className="truncate text-small text-text">
                    {message}
                ...
                {ts && !Number.isNaN(ts.getTime()) && (
                  <span className="shrink-0 font-mono text-caption text-text-faint">
    ...
    <Card className="mb-4 overflow-hidden border-l-2 border-l-level-error p-0">
      <div className="flex flex-wrap items-center justify-between gap-2 px-4 py-2.5">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          {suspect.inApp && (
            <span className="rounded bg-accent/15 px-1.5 py-0.5 text-caption text-accent">
              in-app
            </span>
          )}
        </div>
        ...
        <pre className="overflow-x-auto border-t border-border bg-bg px-0 py-1.5 font-mono text-mono leading-6">
          {(suspect.preContext ?? []).map((l, i) => (
            <SuspectLine
              ...
              n={
                suspect.lineno != null
                  ? suspect.lineno - (suspect.preContext!.length - i)
                  : null
              }
              text={l}
            />
          ))}
          {suspect.contextLine != null && (
            <SuspectLine
              n={suspect.lineno ?? null}
              text={suspect.contextLine}
              crash
            />
          )}
          {(suspect.postContext ?? []).map((l, i) => (
            <SuspectLine
              key={`post-${i}`}
              n={suspect.lineno != null ? suspect.lineno + i + 1 : null}
              text={l}
            />
          ))}
        </pre>
...
function SuspectLine({
  ...
  return (
    <div className={`flex ${crash ? "bg-level-error/10" : ""}`}>
      <span
        className={`inline-block w-12 shrink-0 select-none px-3 text-right ${crash ? "text-level-error" : "text-text-faint"}`}
      >
        {n ?? ""}
      </span>
      <span
        className={`w-3 shrink-0 ${crash ? "text-level-error" : "text-transparent"}`}
      >
        ▸
      </span>
      <span
        className={`whitespace-pre pr-4 ${crash ? "text-text" : "text-text-muted"}`}
      >
        {text}
      </span>
    </div>
...
function EventsChart({
  ...
  return (
    <div
      className="flex h-20 items-end justify-start gap-1"
  ...
  return (
    <Card className="p-4">
      <div className="mb-2 flex items-center justify-between">
        ...
        <span className="rounded bg-surface-2 px-1.5 py-0.5 text-caption text-text-faint">
          AI · Unverified
        </span>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-small text-text-muted">
          <p className="mb-2">
            Get an AI diagnosis of the probable root cause and a fix, grounded
            in this issue's stack trace.
          </p>
          <Button
            size="sm"
            variant="secondary"
        ...
        <div className="flex flex-col gap-2.5 text-small">
          <div className="flex items-center gap-2">
            <span
              className={`rounded px-1.5 py-0.5 text-caption font-medium ${CONF_TONE[data.confidence] ?? CONF_TONE.low}`}
            >
          ...
          <div>
            <div className="mb-0.5 text-caption uppercase tracking-wide text-text-faint">
              Root cause
            </div>
          ...
          {data.explanation && (
            <p className="text-text-muted">{data.explanation}</p>
          )}
          ...
          {(data.patches?.length ?? 0) > 0 &&
            data.patches!.map((p, i) => (
              <div key={i}>
                <div className="mb-0.5 font-mono text-caption text-text-faint">
                  {p.path}
                </div>
          ...
          {hasPatch && (
            <div className="mt-1 border-t border-border pt-2.5">
              {prUrl ? (
                ...
                <span className="text-caption text-text-faint">
                  An admin can open a draft PR from this patch.
                </span>
              ) : prEnabled ? (
                <div className="flex flex-col gap-1">
                  <Button
                    size="sm"
                    variant="secondary"
                    onClick={() => data && onOpenPr(data.id)}
                    disabled={prPending}
                  >
                    {prPending ? "Opening…" : "Open draft PR →"}
                  </Button>
                  <span className="text-caption text-text-faint">
                    Applies the patch to a new branch as a draft. Never
                    auto-merged.
                  </span>
                </div>
              ) : (
                <div className="flex flex-col gap-1">
                  <span className="text-caption text-text-faint">
                    Draft PRs are off for this repo.
                  </span>
                  <button
                  ...
                  >
                    {togglePending
                      ? "Enabling…"
                      : "Enable draft PRs for this repo"}
                  </button>
                </div>
              )}
            </div>
