...
export class GithubService {
  /** GitHub App connected by a project — FR-GH-1. */
  async appForProject(projectId: string): Promise<AppCreds | null> {
    const rows = await db.select().from(githubApps).where(eq(githubApps.projectId, projectId)).limit(1);
  ...
  /**
   * Exchange an installation id for a token using the project's GitHub App.
   * Returns null if no app is connected for this project.
   */
  async installationTokenForProject(projectId: string, installationId: string): Promise<string | null> {
    const creds = await this.appForProject(projectId);
